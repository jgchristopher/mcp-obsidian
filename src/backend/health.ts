 ...
 */
function reduceToFingerprint(names: Iterable<string>): string[] {
  const out: string[] = [];
...
export async function fingerprintRest(client: RestClient): Promise<string[]> {
  ...
  const payload = JSON.parse(res.body) as { files?: unknown };
  const files = Array.isArray(payload.files) ? payload.files : [];
  return reduceToFingerprint(files.filter((name): name is string => typeof name === "string"));
}
...
export class Health implements HealthGate {
  private readonly fingerprintTtlMs: number;
  private readonly unreachableTtlMs: number;
  ...
  async restUsable(): Promise<boolean> {
    ...
    if (this.unreachableSince !== null) {
      if (now - this.unreachableSince < this.unreachableTtlMs) return false;
    ...
    if (now - this.lastCheckAt < this.fingerprintTtlMs) return this.lastResult;
  ...
  private record(result: boolean): boolean {
  ...
  private async check(): Promise<boolean> {
    ...
    try {
      ...
      if (!(error instanceof BackendError)) {
        this.warn(
          `Vault fingerprint check could not read the Obsidian vault listing: ${describe(error)}. ` +
    ...
    try {
      ...
      this.warn(
        `Vault fingerprint check could not read ${this.vaultPath}: ${describe(error)}. ` +
    ...
    if (remote.length === 0 && local.length === 0) {
      this.warn(
        `Vault fingerprint is inconclusive: neither the Obsidian Local REST API nor ` +
    ...
    if (remote.length !== local.length || remote.some((name, i) => name !== local[i])) {
      this.warn(
        `Vault fingerprint mismatch: Obsidian's Local REST API is serving a different vault ` +
   ...
   */
  private async checkPluginVersionOnce(): Promise<void> {
