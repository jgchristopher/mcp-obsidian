...
function parseProtocol(raw: string | undefined): "http" | "https" {
  if (raw === undefined || raw.trim() === "") return DEFAULT_PROTOCOL;
  const value = raw.trim().toLowerCase();
 ...
 */
export function resolveRestConfig(env: NodeJS.ProcessEnv = process.env): RestConfig | null {
 ...
 */
export function isPluginVersionSupported(version: string | undefined | null): boolean {
...
export function pluginVersionWarning(version: string | undefined | null): string | null {
  if (isPluginVersionSupported(version)) return null;
  return (
    `Obsidian Local REST API plugin version ${version} is below the tested floor ` +
