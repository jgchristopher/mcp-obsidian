...
import type { RestClient, RestResponse, SendOptions } from "./rest/client.js";
...
export class ObsidianLiveService {
   ...
   */
  constructor(
    private readonly client: RestClient | null,
    options: ObsidianLiveOptions = {},
  ) {
  ...
  async listCommands(): Promise<ObsidianCommand[]> {
    ...
    const payload = ObsidianLiveService.parse<{ commands?: unknown }>("list_commands", res);
    const commands = Array.isArray(payload.commands) ? payload.commands : [];

    return commands
      .filter((entry): entry is { id: string; name?: unknown } => {
        return typeof entry === "object" && entry !== null && typeof (entry as any).id === "string";
      })
      .map((entry) => ({
        id: entry.id,
   ...
   */
  async executeCommand(id: unknown): Promise<{ success: boolean; id: string }> {
    ...
    if (res.status === 404) {
      throw new Error(
        `Unknown Obsidian command: ${trimmed}. Use list_commands to see the registered ids.`,
      );
    }
  ...
  async getActiveFile(): Promise<{ path: string }> {
    ...
    const payload = ObsidianLiveService.parse<{ path?: unknown }>("get_active_file", res);
  ...
  async openFile(inputPath: unknown): Promise<{ success: boolean; path: string }> {
   ...
   */
  async searchVaultAdvanced(query: unknown): Promise<unknown[]> {
    ...
    const payload = ObsidianLiveService.parse<unknown>("search_vault_advanced", res);
   ...
   */
  private async send(tool: string, opts: SendOptions): Promise<RestResponse> {
    if (!this.client) throw new ObsidianUnavailableError(tool);

    let res: RestResponse;
    try {
      res = await this.client.send(opts);
    } catch (error) {
      if (error instanceof BackendError) throw new ObsidianUnavailableError(tool, error);
      throw error;
    }

    if (res.status === 401 || res.status === 403) throw new RestConfigurationError(res.status);
    return res;
  }

  private static assertOk(tool: string, res: RestResponse, what: string): void {
    if (res.status >= 200 && res.status < 300) return;
    throw new Error(`${tool} failed to ${what}: ${ObsidianLiveService.detail(res)}`);
  }

  private static parse<T>(tool: string, res: RestResponse): T {
    try {
  ...
  private static detail(res: RestResponse): string {
    try {
      ...
      if (parsed.message) return `${parsed.message} (HTTP ${res.status})`;
    } catch {
   ...
   * everywhere else.
   */
  private prepare(inputPath: unknown): string {
    ...
    for (const segment of path.split("/")) {
      ...
      if (depth < 0) {
        throw new Error(
          `Path traversal not allowed: ${path}. Paths must be within the vault directory.`,
        );
      }
    }
    ...
    if (!this.pathFilter.isAllowed(path)) {
      throw new Error(
        `Access denied: ${path}. This path is restricted (system files like .obsidian, .git, and dotfiles are not accessible).`,
      );
    }
